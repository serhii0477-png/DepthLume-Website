...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    const name = text(body.name, 'Ім’я', 2, 80); const userEmail = email(body.email); const userPassword = password(body.password);
    const exists = await env.DB.prepare('SELECT id FROM users WHERE email=?1').bind(userEmail).first();
    if (exists) return json({ ok: true, message: 'Якщо цей email доступний, перевірте пошту.' });
    const now = new Date().toISOString(); const userId = crypto.randomUUID(); const rawToken = randomToken();
    ...
    const sent = await sendAccessEmail(env, userEmail, 'Підтвердження email DepthLume', `<p>Підтвердьте email: <a href="${verifyUrl}">${verifyUrl}</a></p>`);
