...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const row = await env.DB.prepare(`SELECT l.id license_id,l.user_id,u.email,u.email_verified,u.access_status,u.commercial_enabled_at,l.type,l.status license_status,l.starts_at,l.expires_at,l.max_devices,'active' key_status,l.access_type,l.billing_status,l.cancel_at_period_end,a.id activation_id,a.device_id_hash FROM desktop_sessions s JOIN device_activations a ON a.id=s.device_activation_id JOIN licenses l ON l.id=a.license_id JOIN users u ON u.id=l.user_id WHERE s.token_hash=?1 AND s.expires_at>?2 AND s.revoked_at IS NULL AND a.revoked_at IS NULL AND l.product=?3 LIMIT 1`).bind(await sha256(token), now, RADAR_PRODUCT).first<SessionRow>();
